...
except ImportError:  # not installed in the in-browser (Pyodide) build
    Groq = None
...
# In the browser (WebAssembly, unoptimised BLAS) heavy models are ~10x slower than native.
MAX_COMPARE_TRAIN_ROWS = 1500 if IN_BROWSER else 4000
# Diagnostics refit models many times (cross-validation) or predict many times (permutation), so cap harder.
MAX_DIAGNOSTIC_ROWS = 400 if IN_BROWSER else 600
...
class AskRequest(BaseModel):
    ...
    # Sent by the in-browser engine, whose sessions live in the browser, not on this server.
...
@app.get("/pipeline/{session_id}/context")
def session_context(session_id: str):
    """Short text summary of the session, used to ground the AI assistant."""
    session = get_session(session_id)
    return {"context": _session_context_summary(session) if session else None}
...
def ask(body: AskRequest, request: Request):
    ...
    try:
        ...
        session_context = (
            body.session_context[:1000]
            if body.session_context
            else (_session_context_summary(session) if session else None)
        )
        if session_context:
