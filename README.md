...
> The ML backend runs **in your browser** (Python via Pyodide, in a Web Worker), so there is no server to wake up. The first visit downloads about 30 MB of scientific libraries (cached afterwards) and the engine boots in the background while you browse. Only the AI assistant calls a hosted server, because it needs a secret API key.
...
- `backend/` is a FastAPI app. The same code runs two ways: as a normal server (`uvicorn main:app`, used by the tests and as a fallback) and inside the browser.
- In the browser, `frontend/public/py-worker.js` boots Pyodide, mounts the files from `frontend/public/py/` (generated by `npm run sync:backend`, committed) and answers HTTP-like requests through a small ASGI adapter (`backend/pyodide_runner.py`). `frontend/lib/api.ts` exposes it as `apiFetch`, with `fetch` semantics including the streamed Compare All response.
- Built-in dataset lists and previews are static JSON, so the picker is instant while the engine boots.
- If the engine cannot start, requests fall back to the hosted server. Set `NEXT_PUBLIC_ENGINE=server` to always use the server.
- CI runs the backend tests, checks `public/py` is in sync, and runs the shipped backend in real Pyodide (`node scripts/pyodide-smoke.mjs`).
- WebAssembly is slower than native for heavy models, so in the browser the comparison/diagnostics use smaller row caps and neural networks train fewer iterations.
